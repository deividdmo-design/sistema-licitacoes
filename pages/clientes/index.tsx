...
export default function ListaClientes() {
  ...
  const [filtroStatus] = useState('Todos') // pode ser usado depois
  ...
  const clientesFiltrados = clientes.filter(cli => {
    ...
    return (
      (cli.razao_social || '').toLowerCase().includes(termoBusca) ||
      (cli.cnpj || '').toLowerCase().includes(termoBusca) ||
      (cli.gestor_contrato || '').toLowerCase().includes(termoBusca)
    )
  })
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', color: 'white', padding: '25px', borderRadius: '16px', boxShadow: '0 10px 15px -3px rgba(15, 23, 42, 0.3)', position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ fontSize: '2.8rem', fontWeight: '800' }}>{clientes.length}</div>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '900px' }}>
              ...
              <tbody>
                {clientesFiltrados.map((cli) => (
                  <tr key={cli.id} style={{ borderBottom: '1px solid #f1f5f9', transition: 'background 0.2s' }} onMouseOver={(e) => e.currentTarget.style.background = '#f8fafc'} onMouseOut={(e) => e.currentTarget.style.background = 'transparent'}>
                    
                    <td style={{ padding: '16px 20px', maxWidth: '350px' }}>
                      <strong style={{ color: '#0f172a', display: 'block', fontSize: '0.95rem' }}>{cli.razao_social}</strong>
                    </td>
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      <div style={{ color: '#0369a1', fontSize: '0.9rem', marginBottom: '2px' }}>{cli.email || '-'}</div>
                      <div style={{ color: '#64748b', fontSize: '0.85rem' }}>{cli.telefone || '-'}</div>
                    </td>

                    <td style={{ padding: '16px 20px', color: '#334155', fontWeight: '500' }}>
                      {cli.gestor_contrato ? (
                        <span style={{ background: '#f1f5f9', padding: '4px 10px', borderRadius: '6px', fontSize: '0.85rem' }}>👤 {cli.gestor_contrato}</span>
                      ) : (
                    ...
                    <td style={{ padding: '16px 20px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                        <Link href={`/clientes/editar/${cli.id}`}>
                        ...
                        <button onClick={() => excluirCliente(cli.id)} title="Excluir Instituição" style={{ background: '#fee2e2', border: 'none', padding: '8px', borderRadius: '6px', cursor: 'pointer', transition: 'transform 0.1s' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.1)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}>
                          🗑️
                        </button>
                      </div>
                    </td>
